...
//----------------------------------------------------------------------
Real DL = 0.01;                       /**< Tank length. */
Real DH = 0.002;                      /**< Tank height. */
Real LL = DL;                       /**< Soil column length. */
Real LH = DH;                       /**< Soil column height. */
Real particle_spacing_ref = DH/25.0; /**< Initial reference particle spacing. */
Real BW = particle_spacing_ref * 4;  /**< Extending width for boundary conditions. */
BoundingBox system_domain_bounds(Vec2d(-BW, -BW), Vec2d(DL + BW, DH + BW));
// observer location
StdVec<Vecd> observation_location = {Vecd(0.5*DL, DH * 0.5)};
//----------------------------------------------------------------------
...
Real segregation_rate = 0.0;
Real initial_temperature = 20.0;
Real heat_flux = 0.0; //from the Nemann boundary
Real lower_temperature = 40.0;
Real upper_temperature = 20.0;
//----------------------------------------------------------------------
...
Vec2d wall_halfsize = Vec2d(0.5 * DL + BW, 0.5 * BW);
Vec2d lower_outer_wall_translation = Vec2d(-BW, -BW) + wall_halfsize;
Vec2d upper_outer_wall_translation = Vec2d(-BW, DH) + wall_halfsize;



std::vector<Vecd> lower_region_edge_points{
    Vecd(-BW, -BW), Vecd(-BW, 0.0), Vecd(DL+BW, 0.0),
    Vecd(DL+BW, -BW), Vecd(-BW, -BW)};
std::vector<Vecd> upper_region_edge_points{
    Vecd(-BW, DH), Vecd(-BW, DH + BW), Vecd(DL+BW, DH + BW),
    Vecd(DL+BW, DH), Vecd(-BW, DH)};
//----------------------------------------------------------------------
