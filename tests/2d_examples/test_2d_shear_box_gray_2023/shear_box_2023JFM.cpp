...
int main(int ac, char *av[])
{
    ...
    /*Shear making*/
    BodyRegionByParticle soil_sheaking_maker(soil_block, makeShared<MultiPolygonShape>(createSoilShearShape()));
    SimpleDynamics<ShearMaking> soil_shear_making(soil_sheaking_maker);
    BodyRegionByParticle wall_sheaking_maker(wall_boundary, makeShared<MultiPolygonShape>(createShearShape()));
    SimpleDynamics<ShearMaking> wall_shear_making(wall_sheaking_maker);
    ...
    //SimpleDynamics<UpdateDiffusivityAndSegregationRate> update_D_and_V(soil_block, B/0.087);
    SimpleDynamics<UpdateDiffusivityAndSegregationRateGray2023NonDim> update_D_and_V(soil_block, 0.087, 1.0);
    ...
    write_states.addToWrite<Vecd>(soil_block, "NormalDirection");
    write_states.addToWrite<Vecd>(soil_block, "SegNormalDirection");
    write_states.addToWrite<Vecd>(soil_block, "InitialPosition");
    ...
    int ite = 0;
    Real End_Time = 150 * shear_period;
    Real Output_Time = shear_period/5.0;
    Real dt = 0.0;
    //----------------------------------------------------------------------
    ...
    while (physical_time < End_Time)
    {
        ...
        while (integration_time < Output_Time)
        {
            Real relaxation_time = 0.0;
                ...
                soil_shear_making.exec();
                wall_shear_making.exec();
