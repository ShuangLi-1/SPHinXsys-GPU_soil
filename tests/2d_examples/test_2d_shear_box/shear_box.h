...
//----------------------------------------------------------------------
Real LH = 1.0;                       /**< Soil column height. */
Real LL = LH*0.577;                       /**< Soil column length. */
...
BoundingBox system_domain_bounds(Vec2d(-DL- BW, -BW), Vec2d(2.0*DL + BW, DH + BW));
// observer location
...
Real Sr = 0.03;
Real Dr = Sr/29.6;
Real segregation_rate = Sr * shear_period / LH;
Real diffusion_coeff =  Dr* shear_period / LH/LH;
Real initial_temperature = 0.5;
Real heat_flux = 0.0; //from the Nemann boundary
Real miu_s = tan(friction_angle);
Real miu_d = tan(friction_angle);
Real d_min = 0.004;
Real d_max = 0.008;
//----------------------------------------------------------------------
...
Vec2d base_wall_halfsize = Vec2d(DL, 0.5 * BW);
Vec2d base_wall_translation = base_wall_halfsize + Vec2d(0.0, -BW);
...
class WallBoundary : public ComplexShape
{
  public:
    explicit WallBoundary(const std::string &shape_name) : ComplexShape(shape_name)
    {
        ...
        add<TransformShape<GeometricShapeBox>>(Transform(base_wall_translation), base_wall_halfsize);
