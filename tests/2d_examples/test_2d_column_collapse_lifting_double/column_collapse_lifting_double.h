...
Real DH = 0.4;                      /**< Tank height. */
Real LL = 0.08;                       /**< Soil column length. */
Real LH = 0.158;                       /**< Soil column height. */
Real LL2 = LL;                       /**< Gate length. */
Real LH2 = 0.054;                       /**< Gate height. */
Real Distance = 0.04;
Real particle_spacing_ref = LH / 80; /**< Initial reference particle spacing. */
Real BW = particle_spacing_ref * 4;  /**< Extending width for boundary conditions. */
BoundingBox system_domain_bounds(Vec2d(-BW, -BW), Vec2d(DL + BW, DH + BW));
//----------------------------------------------------------------------
//	Material properties of the soil.
//----------------------------------------------------------------------
Real rho0_s = 1550;                                                       // reference density of soil
...
Real friction_angle = 32.9 * Pi / 180;
Real cohesion = 0.0;
Real dilatancy = 0.0;
Real miu_s = 0.384;
Real miu_d = 0.625;
...
Vec2d soil_block_translation = soil_block_halfsize + Vecd(0.4,0.0); 
Vec2d soil_block_halfsize2 = Vec2d(0.5 * LL2, 0.5 * LH2); // local center at origin:
Vec2d soil_block_translation2 = soil_block_halfsize2 + Vecd(0.4,0.0) + Vecd(Distance + 0.5 * LL + 0.5 * LL2,0.0); 
...
Vec2d outer_gate_halfsize2 = Vecd(0.5 * LL2 + BW, 0.5 * LH2);
Vec2d outer_gate_translation2 = soil_block_translation2;
Vec2d inner_gate_halfsize2 = Vecd(0.5 * LL2, 0.5 * LH2);
Vec2d inner_gate_translation2 = soil_block_translation2;
...
class WallBoundary : public ComplexShape
{
  public:
    explicit WallBoundary(const std::string &shape_name) : ComplexShape(shape_name)
    {
        ...
        // add<TransformShape<GeometricShapeBox>>(Transform(outer_gate_translation2), outer_gate_halfsize2);
        // subtract<TransformShape<GeometricShapeBox>>(Transform(inner_gate_translation2), inner_gate_halfsize2);
    }
};
...
class SoilBlock : public ComplexShape
{
  public:
    explicit SoilBlock(const std::string &shape_name) : ComplexShape(shape_name)
    {
      ...
      add<TransformShape<GeometricShapeBox>>(Transform(soil_block_translation2), soil_block_halfsize2);
    }
};
...
MultiPolygon createLiftingShape()
{
    ...
    multi_polygon.addABox(Transform(outer_gate_translation2), outer_gate_halfsize2, ShapeBooleanOps::add);
...
class Lifting : public BodyPartMotionConstraint
{
  ...
  public:
    explicit Lifting(BodyPartByParticle &body_part, Real a_up = 64.0)
