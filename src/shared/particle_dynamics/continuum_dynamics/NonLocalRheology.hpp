...
NonLocalPlasticIntegration1stHalf<Inner<>, RiemannSolverType>::
    NonLocalPlasticIntegration1stHalf(BaseInnerRelation &inner_relation)
    : BasePlasticIntegration<DataDelegateInner>(inner_relation),
      riemann_solver_(plastic_continuum_, plastic_continuum_) {}
//=================================================================================================//
template <class RiemannSolverType>
void NonLocalPlasticIntegration1stHalf<Inner<>, RiemannSolverType>::initialization(size_t index_i, Real dt)
{
...
//=================================================================================================//
template <class RiemannSolverType>
Vecd NonLocalPlasticIntegration1stHalf<Inner<>, RiemannSolverType>::computeNonConservativeForce(size_t index_i)
{
...
//=================================================================================================//
template <class RiemannSolverType>
void NonLocalPlasticIntegration1stHalf<Inner<>, RiemannSolverType>::interaction(size_t index_i, Real dt)
{
...
//=================================================================================================//
template <class RiemannSolverType>
void NonLocalPlasticIntegration1stHalf<Inner<>, RiemannSolverType>::update(size_t index_i, Real dt)
{
    vel_[index_i] += (force_prior_[index_i] + force_[index_i]) / mass_[index_i] * dt;
}
//=================================================================================================//
template <class RiemannSolverType>
NonLocalPlasticIntegration1stHalf<Contact<Wall>, RiemannSolverType>::
    NonLocalPlasticIntegration1stHalf(BaseContactRelation &wall_contact_relation)
    : BaseIntegrationWithWall(wall_contact_relation),
      riemann_solver_(plastic_continuum_, plastic_continuum_) {}
//=================================================================================================//
template <class RiemannSolverType>
void NonLocalPlasticIntegration1stHalf<Contact<Wall>, RiemannSolverType>::interaction(size_t index_i, Real dt)
{
...
//=================================================================================================//
template <class RiemannSolverType>
Vecd NonLocalPlasticIntegration1stHalf<Contact<Wall>, RiemannSolverType>::computeNonConservativeForce(size_t index_i)
{
    return this->force_prior_[index_i];
}
//=================================================================================================//
template <class RiemannSolverType>
NonLocalPlasticIntegration2ndHalf<Inner<>, RiemannSolverType>::NonLocalPlasticIntegration2ndHalf(BaseInnerRelation &inner_relation)
    : BasePlasticIntegration<DataDelegateInner>(inner_relation),
      riemann_solver_(plastic_continuum_, plastic_continuum_, 20.0 * (Real)Dimensions),
      Vol_(particles_->getVariableDataByName<Real>("VolumetricMeasure")),
      mass_(particles_->getVariableDataByName<Real>("Mass")),
      concentration_(particles_->registerStateVariable<Real>("Concentration")) {}
//=================================================================================================//
template <class RiemannSolverType>
void NonLocalPlasticIntegration2ndHalf<Inner<>, RiemannSolverType>::initialization(size_t index_i, Real dt)
{
    pos_[index_i] += vel_[index_i] * dt * 0.5;
}
//=================================================================================================//
template <class RiemannSolverType>
void NonLocalPlasticIntegration2ndHalf<Inner<>, RiemannSolverType>::interaction(size_t index_i, Real dt)
{
    Real density_change_rate(0);
    Vecd p_dissipation = Vecd::Zero();
    Matd velocity_gradient = Matd::Zero();
    Real nonlocal_fluidity_rate_i = 0.0;
    Real alpha_each_i(0.0), kc_each_i(0.0);
    Real sum_w(0.0);
    /*Nonlocal diffusion*/
    Real d_i = concentration_[index_i] * d_min_ + (1-concentration_[index_i])*d_max_;
    //Real d_i = 0.002;
    Real diff_coeff_ij = 1.0 * 1.0 * d_i * d_i;
    const Neighborhood &inner_neighborhood = inner_configuration_[index_i];
    for (size_t n = 0; n != inner_neighborhood.current_size_; ++n)
    {
        size_t index_j = inner_neighborhood.j_[n];
        ...
        /*Nonlocal diffusion*/
        const Vecd &grad_ijV_j = dW_ijV_j * e_ij;
        Real surface_area_ij = 2.0 * grad_ijV_j.dot(e_ij) / r_ij_;
        Real phi_ij = fluidity_[index_i] - fluidity_[index_j];
        nonlocal_fluidity_rate_i += diff_coeff_ij * phi_ij * surface_area_ij;

        /*Smoothing DP parameters*/
        W_ij = SMAX(W_ij, 0.0);
        alpha_each_i += alpha_each_[index_i]* W_ij;
        kc_each_i += Kc_each_[index_i] * W_ij;
        sum_w += W_ij;
    }
    ...
    /*Nonlocal diffusion*/
    nonlocal_fluidity_rate_[index_i] = nonlocal_fluidity_rate_i;
    /*Smoothing DP parameters*/
    // alpha_each_[index_i] = alpha_each_i/sum_w;
    // Kc_each_[index_i] = kc_each_i/sum_w;

}
//=================================================================================================//
template <class RiemannSolverType>
void NonLocalPlasticIntegration2ndHalf<Inner<>, RiemannSolverType>::update(size_t index_i, Real dt)
{   
...
//=================================================================================================//
template <class RiemannSolverType>
NonLocalPlasticIntegration2ndHalf<Contact<Wall>, RiemannSolverType>::
    NonLocalPlasticIntegration2ndHalf(BaseContactRelation &wall_contact_relation)
    : BaseIntegrationWithWall(wall_contact_relation),
      riemann_solver_(plastic_continuum_, plastic_continuum_),
      concentration_(particles_->registerStateVariable<Real>("Concentration")) {}
//=================================================================================================//
template <class RiemannSolverType>
void NonLocalPlasticIntegration2ndHalf<Contact<Wall>, RiemannSolverType>::interaction(size_t index_i, Real dt)
{
    ...
    for (size_t k = 0; k < contact_configuration_.size(); ++k)
    {
        ...
        for (size_t n = 0; n != wall_neighborhood.current_size_; ++n)
        {
            ...
            Vecd &e_ij = wall_neighborhood.e_ij_[n];
            Real dW_ijV_j = wall_neighborhood.dW_ij_[n] * wall_Vol_k[index_j];
            Real r_ij_ = wall_neighborhood.r_ij_[n];
            ...
            /*Nonlocal diffusion*/
            //Real diff_coeff_ij = 1.0;
            const Vecd &grad_ijV_j = dW_ijV_j * e_ij;
            Real surface_area_ij = 2.0 * grad_ijV_j.dot(e_ij) / r_ij_;
            Real phi_ij = fluidity_[index_i] - 0.0;
            nonlocal_fluidity_rate_i += diff_coeff_ij * phi_ij * surface_area_ij;
        }
    }
    drho_dt_[index_i] += density_change_rate * rho_[index_i];
